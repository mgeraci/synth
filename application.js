...
(function() {
  var TIME_CONSTANT, controlWatchers, init, keyboard, note, noteOff, noteOn, oscillator, setDefaults;
  ...
  oscillator = function() {
    ...
    this.osc.start(); // Start generating sound immediately
    return this.osc.frequency.setTargetAtTime(500, this.context.currentTime, TIME_CONSTANT);
  };

  // set the frequency and trigger the attack
  note = function(freq) {
    this.osc.frequency.setTargetAtTime(freq, this.context.currentTime, TIME_CONSTANT);
    return noteOn();
  };
...
// attack
    for (i = j = 0, ref = aNumberOfSteps; (0 <= ref ? j < ref : j > ref); i = 0 <= ref ? ++j : --j) {
      this.aInnerTimeout = setTimeout(() => {
        ...
        return this.gainnode.gain.setValueAtTime(this.output, this.context.currentTime);
      }, i * this.grain);
    }
    // decay
